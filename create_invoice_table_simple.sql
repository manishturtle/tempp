...
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_client_id ON invoices_invoice(client_id);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_account_id ON invoices_invoice(account_id);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_order_id ON invoices_invoice(order_id);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_invoice_number ON invoices_invoice(invoice_number);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_date ON invoices_invoice(date);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_due_date ON invoices_invoice(due_date);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_invoice_status ON invoices_invoice(invoice_status);
CREATE INDEX IF NOT EXISTS idx_invoices_invoice_payment_status ON invoices_invoice(payment_status);

-- Add constraint for unique invoice number when not cancelled
DROP INDEX IF EXISTS unique_invoice_number_if_not_cancelled;
