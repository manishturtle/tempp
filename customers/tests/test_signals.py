...
class TestAccountContactSyncSignal(TestCase):
    ...
    @patch('customers.signals.sync_individual_account_contact_data')
    def test_signal_triggered_for_individual_account_name_update(self, mock_sync_task):
        """Test that the signal is triggered when an Individual account's name is updated."""
        # Update an Individual account's name with update_fields
        self.individual_account.name = "John Smith"
    ...
    @patch('customers.signals.sync_individual_account_contact_data')
    def test_signal_not_triggered_for_business_account_name_update(self, mock_sync_task):
        """Test that the signal is not triggered when a Business account's name is updated."""
        # Update a Business account's name with update_fields
        self.business_account.name = "Acme Corporation"
        self.business_account.save(update_fields=['name'])
    ...
    @patch('customers.signals.sync_individual_account_contact_data')
    def test_signal_not_triggered_for_business_contact_name_update(self, mock_sync_task):
        """Test that the signal is not triggered when a Business contact's name is updated."""
        # Update a Business contact's name with update_fields
        self.business_contact.first_name = "Janet"
        self.business_contact.save(update_fields=['first_name'])
