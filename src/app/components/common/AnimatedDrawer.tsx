import React, { useState, useEffect, ReactNode, MouseEvent, createContext, useContext, useMemo, useCallback } from 'react';
import { 
  Box, 
  Drawer, 
  Typography, 
...
const EventControlsContext = createContext<EventControlsContextType | null>(null);
...
const AnimatedDrawer: React.FC<AnimatedDrawerProps> = ({
  ...
  const [dynamicEventContent, setDynamicEventContent] = useState<ReactNode>(null);
  ...
  const calculateDrawerWidth = () => {
    ...
    if (isEventSectionOpen && (event || dynamicEventContent || enableEventSection)) {
  ...
  }), [isEventSectionOpen, toggleEventSection, onEventContentChange, onEventToggle]);
            ...
            <Box 
              sx={(theme) => ({
                ...
                transition: theme.transitions.create('opacity', {
                  duration: theme.transitions.duration.standard,
                  easing: theme.transitions.easing.easeInOut,
                  delay: isEventSectionOpen ? theme.transitions.duration.shorter : 0,
                }),
                p: theme.spacing(2),
                overflow: 'auto',
              })}
        ...
        >
          ...
          <Box sx={(theme) => ({ 
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
              ...
              {(event || dynamicEventContent || enableEventSection) && (
                <Tooltip title={isEventSectionOpen ? "Hide Events" : "Show Events"} placement="bottom">
                  <IconButton 
                    onClick={toggleEventSection}
                    size="small"
                    aria-label="Toggle event section"
                    sx={(theme) => ({
                      color: isEventSectionOpen ? theme.palette.primary.main : 'inherit',
