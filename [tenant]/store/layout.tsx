...
export default function StoreLayout({
  ...
  function SegmentDetailsCache() {
    ...
    // On mount, check localStorage for both segment details and customer group ID (from auth_user)
    useEffect(() => {
      if (!segmentKey || !authUserKey) return;
      const existingSegment = localStorage.getItem(segmentKey);
      const existingAuthUser = localStorage.getItem(authUserKey);
      if (existingSegment && existingAuthUser) {
        try {
          setSegmentDetails(JSON.parse(existingSegment));
          const authUserObj = JSON.parse(existingAuthUser);
          setCustomerGroupId(authUserObj.customer_group_id || null);
        } catch (e) {
          console.error('Error parsing segment details or customer_group_id from auth_user in localStorage', e);
        }
      }
    }, [segmentKey, authUserKey]);

    // Compare localStorage customer_group_id with segmentDetails.customer_group_id (from auth_user)
    let customerGroupIdFromStorage = null;
    if (typeof window !== 'undefined') {
      const existingAuthUser = localStorage.getItem(authUserKey);
      if (existingAuthUser) {
        try {
          const authUserObj = JSON.parse(existingAuthUser);
          customerGroupIdFromStorage = authUserObj.customer_group_id || null;
        } catch (e) {
          console.error('Error parsing customer_group_id from auth_user in localStorage', e);
        }
      }
    }
    ...
    useEffect(() => {
      if (isSuccess && segmentData && segmentKey && (shouldFetch || !segmentDetails)) {
        ...
        if (segmentData.customer_group_id) {
          // Update customer_group_id inside auth_user object
          const existingAuthUser = localStorage.getItem(authUserKey);
          if (existingAuthUser) {
            try {
              const authUserObj = JSON.parse(existingAuthUser);
              authUserObj.customer_group_id = segmentData.customer_group_id;
              localStorage.setItem(authUserKey, JSON.stringify(authUserObj));
              setCustomerGroupId(segmentData.customer_group_id);
            } catch (e) {
              console.error('Error updating customer_group_id in auth_user in localStorage', e);
            }
          }
        }
        setSegmentDetails(segmentData);
      }
    }, [isSuccess, segmentData, segmentKey, segmentDetails, shouldFetch, authUserKey]);
    return null;
  }
