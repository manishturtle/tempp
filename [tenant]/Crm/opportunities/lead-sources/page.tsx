...
import {
  ...
  useDeleteOpportunityLeadSource,
  useToggleOpportunityLeadSourceStatus,
...
// Main component for the opportunity lead sources list page
function OpportunityLeadSourcesPage() {
  ...
  const [filters, setFilters] = useState<Record<string, any>>({
    // Not setting status by default to show all lead sources
  });
  ...
  // State for selected lead source (for edit mode)
  const [selectedLeadSource, setSelectedLeadSource] = useState<OpportunityLeadSource | null>(
    null
  );

  // Fetch opportunity lead sources using the custom hook
  const {
  ...
  const toggleActiveMutation = useToggleOpportunityLeadSourceStatus();
  ...
  // Mutation for deleting a lead source
  const deleteOpportunityLeadSource = useDeleteOpportunityLeadSource();
  ...
  const confirmDeleteAction = useCallback(() => {
    if (confirmDelete.id) {
      deleteOpportunityLeadSource.mutate(confirmDelete.id, {
        onSuccess: () => {
          setConfirmDelete({ open: false, id: null });
          showSuccess(t("leadSourceForm.deleteSuccess", "Lead source deleted successfully"));
        },
        onError: () => {
          showError(t("leadSourceForm.deleteError", "Error deleting lead source"));
        },
      });
    }
  }, [confirmDelete.id, deleteOpportunityLeadSource, showSuccess, showError, t]);
  ...
  // Handler for editing a lead source
  const handleEdit = useCallback((leadSource: OpportunityLeadSource) => {
  ...
  // Handler for viewing a lead source
  const handleView = useCallback((leadSource: OpportunityLeadSource) => {
    setSelectedLeadSource(leadSource);
    setDrawerMode("edit");
    setIsViewMode(true);
    setDrawerOpen(true);
  ...
  // Handler for creating a new lead source
  ...
  const handleFormSubmit = useCallback(() => {
    ...
    showSuccess(t("leadSourceForm.saveSuccess", "Lead source saved successfully"));
  ...
  const filterOptions: FilterOption[] = [
    {
      field: "status",
      label: t("leadSourceFields.status", "Status"),
      type: "select",
      options: [
  ...
  const columnOptions = [
    { field: "name", headerName: t("leadSourceFields.name", "Name") },
    { field: "desc", headerName: t("leadSourceFields.description", "Description") },
    { field: "status", headerName: t("leadSourceFields.status", "Status") },
    { field: "actions", headerName: t("common.actions", "Actions") },
  ];

  // Calculate active and inactive counts from results
  const activeLeadSourcesCount = React.useMemo(() => {
    return leadSourcesData?.results?.filter((leadSource) => leadSource.status).length || 0;
  }, [leadSourcesData?.results]);

  const inactiveLeadSourcesCount = React.useMemo(() => {
    return leadSourcesData?.results?.filter((leadSource) => !leadSource.status).length || 0;
  }, [leadSourcesData?.results]);
  ...
  const tabOptions = [
    {
      ...
      count: leadSourcesData?.count || 0,
    },
    {
      value: "active",
      label: t("filterStatuses.active", "Active"),
      count: activeLeadSourcesCount,
    },
    {
      value: "inactive",
      label: t("filterStatuses.inactive", "Inactive"),
      count: inactiveLeadSourcesCount,
    },
  ];
  ...
  const columns: GridColDef[] = [
    {
      field: "name",
      headerName: t("leadSourceFields.name", "Name"),
      flex: 1,
      minWidth: 200,
    },
    {
      field: "desc",
      headerName: t("leadSourceFields.description", "Description"),
      flex: 2,
      minWidth: 250,
    },
    {
      field: "status",
      headerName: t("leadSourceFields.status", "Status"),
      width: 120,
    ...
    <Box>
      ...
      >
        <Typography variant="h4" component="h1" sx={{ fontWeight: 600 }}>
          {t("leadSourceList.title", "Opportunity Lead Sources")}
        </Typography>
        ...
        >
          {t("leadSourceList.newButton", "New Lead Source")}
        </Button>
      </Box>
      ...
      >
        {isError ? (
          <Typography color="error" sx={{ p: 2 }}>
            {t("leadSourceList.error", "Error loading lead sources")}
          </Typography>
        ) : isLoading ? (
          ...
          <CustomDataGrid
            rows={leadSourcesData?.results || []}
            columns={columns.filter((col) =>
              visibleColumns.includes(col.field)
            )}
            rowCount={leadSourcesData?.count || 0}
            loading={isLoading}
            ...
            onRowClick={(params: any) => {
              handleView(params.row as OpportunityLeadSource);
            }}
          />
        ) : (
          ...
          <Box sx={{ p: 3 }}>
            <Typography variant="body1" color="text.secondary" align="center">
              {t("leadSourceList.gridViewNotImplemented", "Grid view not implemented")}
            </Typography>
          </Box>
        )}
      </ContentCard>
      ...
      <AnimatedDrawer
        ...
        title={
          isViewMode
            ? t("leadSourceForm.viewTitle", "View Lead Source")
            : drawerMode === "add"
            ? t("leadSourceForm.createTitle", "Create Lead Source")
            : t("leadSourceForm.editTitle", "Edit Lead Source")
        }
      ...
      <ConfirmDialog
        ...
        title={t("leadSourceForm.deleteConfirmTitle", "Delete Lead Source")}
        content={t(
          "leadSourceForm.deleteConfirmMessage",
          "Are you sure you want to delete this lead source? This action cannot be undone."
        )}
        onConfirm={confirmDeleteAction}
        onCancel={() => setConfirmDelete({ open: false, id: null })}
        isLoading={deleteOpportunityLeadSource.isPending}
      />
...
export default function OpportunityLeadSourcesPageWrapper() {
  return (
    <DrawerProvider>
